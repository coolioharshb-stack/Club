import { collection, doc, setDoc, getDocs, deleteDoc } from "https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js";

const db = window.db; // Firestore initialized in index.html
...
// Populate dropdowns
window.loadClassOptions = async function() {
  const selects = [
...
// ====== Attendance ======
window.loadAttendanceClasses = function(){
  loadClassOptions();
...
window.loadAttendanceStudents = async function(){
  ...
  snapshot.forEach(sDoc=>{
    ...
    if(s.class===classSel){
      ...
      row.appendChild(btnP); row.appendChild(btnA); listDiv.appendChild(row);
    }
  });
};
...
// ===== Dashboard =====
window.loadDashboardClasses = function(){
  loadClassOptions();
  document.getElementById("dashboardData").innerHTML="";
};

window.loadDashboardStudents = async function(){
